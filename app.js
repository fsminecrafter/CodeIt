...
function closeTab(name){
  const tab=[...tabsEl.children].find(t=>t.dataset.name===name);
  if(tab) tab.remove();
  if(current===name){ current=null; editor.setValue(""); }
}
...
document.getElementById("support").onclick=()=>alert("Supports: Chrome only");
...
worker.onmessage=e=>{ if(e.data.type==="output") termWrite(e.data.text); };
